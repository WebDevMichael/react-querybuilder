...
it('formats to mongo query correctly', () => {
  let mongoQueryString4 = hjson.parse(mongoQueryString);
  const formatMongoQuery4String = formatQuery(mongoQuery, 'mongodb');
  const formatMongoQuery4Json = hjson.parse(formatMongoQuery4String);
  ...
  expect(hjson.parse(formatMongoQuerySourceField)).toEqual(
    hjson.parse(mongoQueryStringForValueSourceField)
  );
  expect(
...
it('formats to mongo query correctly - 1', () => {
  ...
  const mongoQueryString1 = hjson.parse(`{"firstName":"Test"}`);
  const format1 = hjson.parse(formatQuery(mongoQuery1, 'mongodb'));
...
it('formats to mongo query correctly - 2', () => {
  ...
  expect(hjson.parse(formatQuery(mongoQuery2, 'mongodb'))).toEqual(hjson.parse(mongoQueryString2));
});
...
describe('independent combinators', () => {
  ...
  it('handles independent combinators for mongodb', () => {
    ...
    expect(hjson.parse(formatQueryResult)).toEqual(
      hjson.parse(
...
describe('validation', () => {
  ...
  describe('mongodb', () => {
    it('should invalidate a mongodb query', () => {
      ...
      expect(hjson.parse(formattedQueryMongoDb)).toEqual(
        hjson.parse(
          `{"_id":"asd","ACC360Objecttype":"qwert","$or":[{"ACC360Objecttype":"orval"},{"ACC360Objecttype":"orval2"}],"$and":[{"ACC360Objecttype":"and111"},{"ACC360Objecttype":"and222"}]}`
        )
      );
    ...
    it('should invalidate a mongodb rule', () => {
      ...
      ).toEqual(hjson.parse('{"otherfield":""}'));
    });
...
describe('parseNumbers', () => {
  ...
  it('parses numbers for mongodb', () => {
    ...
    expect(hjson.parse(formattedQuery)).toEqual(
      hjson.parse(
